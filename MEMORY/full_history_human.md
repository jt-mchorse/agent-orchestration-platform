...
## 2026-07-17 — Issue #113: guard the eval-runner results write seam (exit 2)

#111 guarded the fixtures read seam (`discoverCases` → clean exit 2) and wired
`main()`'s return code to `process.exit`, but left the results write seam bare:
`atomicWriteFile` on the operator `--results-dir` throws (ENOTDIR when a path
component is a file, EACCES, ENOENT) through the top-level `.catch` as a raw exit-1
traceback. The write runs on every invocation (before the `--dry-run` return), so
it's readily reachable. Wrapped it to emit `::error::cannot write results` and
...
## 2026-09-11 — one predicate for two integer columns and a decimal one (#143, D-015)

**What got done.** `aggregateCost` guarded its three summands with a single
`Number.isFinite(x) && x >= 0`. The three destinations do not share a domain:
`total_input_tokens` and `total_output_tokens` are `BIGINT`, and `total_cost_dollars`
is `NUMERIC(12, 6)`. So a fractional token count was accepted, summed, round-tripped
by `MemoryStore` exactly, and unstorable by `PgStore` — visible only in the
`DATABASE_URL`-gated job. That is the sibling of #141, one function above the fix #142
landed this morning, and #142's own reasoning transfers word for word.

What made #143 findable was #142's own sentence: "Two numeric inputs in one module
should not report differently." That is a claim there are two. There were three, and
the third had the loosest domain while feeding the narrowest columns.

**The two halves deserve opposite treatment.** A fractional token count is corrupt
data — tokens are counted, not measured — so it is skipped, which is the partial-total
posture this aggregator documents. A sub-microcent dollar charge is real money:
skipping it loses it, and rounding each observation to the column's scale rounds it to
zero and loses the same money. So dollars are summed at full precision and the total is
quantised once — in the shared aggregator, not in either backend, because that is what
makes the two agree. `MemoryStore` used to keep the full-precision float while
`PgStore` handed the same float to a column that rounds silently, so `getRun` returned
a different number depending on which backend answered.

**My own test refuted my own argument, which is the best part.** I wrote "a single
sub-microcent charge is NOT skipped" and expected a value above zero. I got zero:
`NUMERIC(12, 6)` has no representation below 5e-7, and the value is zero in the column
whether or not the aggregator quantises. The claim I was entitled to is different and
narrower — the charge *enters the sum*, so ten thousand of them come to 0.001 instead
of 0. That is the real improvement, and the six-decimal floor is now a pinned, known
limitation rather than something I'd have asserted away.

**And the doc lock caught me naming the function I had just deleted.**
`docs/architecture.md` described the removal while still writing `isCountableCost` in
backticks, and the symbol lock correctly refused a backticked name that no longer
resolves. Reworded to name it in prose.

**Why this was prioritized.** Both of this repo's open issues are JT-gated
decision-revisits, so the work came from hunting, and the diff merged two hours earlier
in this same run was the richest surface — for the third time this session.
